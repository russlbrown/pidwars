...
def generate_bot(team, x, y, color, beam_color1, beam_color2,
    ...
    bot_template = {
        ...
        'setpoint_x' : x,
        'setpoint_y' : y, 
        'max_thrust' : 0.03, #0.3 works nicely
        'nearest_enemy' : -1,
...
def main():
    ...
    players_per_team = 100//number_of_teams   #set the number of players per team
    field_length = 60
    field_width = 40 
    sight_range = 10
    ...
    dt = 1.0
    AirDragCoeff = 0.99 #max speed = max_thrust * (1/airDrag - 1)
    
    attack_range = 7.0
    attack_delay = 20 #frames between attack

    wall_bounce = -0.5 #-1 means rebound with the same velocity
    ...
    for i in range(players_per_team):
        for j in range(number_of_teams):
            ...
            bot.append(
                generate_bot(
                    j + 1,
                    random.uniform(0, field_length / number_of_teams) + (field_length * j / number_of_teams),
                    random.uniform(0,
                                   field_width),
                    color_by_team[j],
    ...
    while True:
        ...
        for i in range(len(bot)):
            if bot[i]['is_alive']:
                ...
                if nearest_enemy != -1: #if enemies are in range
                    ...
                    if nearest_friend_in_combat != -1: #approach nearest friend with enemies in range
                        ...
                        if dist_of_farthest_friend > bot[i]['sight_range']:
                            ...
                            bot[i]['setpoint_x'] = bot[i]['x']
                            bot[i]['setpoint_y'] = bot[i]['y']
                    else: #elif bot[i]['is_roaming'] == False: # start looking around
                        bot[i]['has_setpoint'] = True
                        bot[i]['is_roaming'] = False
                ...
                #else: #the bot has no setpoint and is not roaming. it is retreating
                ...
                if dist_of_nearest_bot < 0.5:
                    output_x += ((bot[i]['x']-bot[nearest_bot]['x'])
                                 /dist_of_nearest_bot)*bot[i]['max_thrust']*1.0
